...
typedef BOOL WINAPI PROC_wglChoosePixelFormatARB(HDC hDC, const int *piAttribIList, const FLOAT *pfAttribFList, UINT nMaxFormats, int *piFormats, UINT *nNumFormats);
...
namespace GlHelper
{
    ...
    HGLRC WglContextManager::CreateByHwnd(
        HWND hWND,
    ...
    HGLRC WglContextManager::CreateByBitmap(void **color_buffer, int width, int height, int major, int minor, int core)
    {
        ...
        BITMAPINFO bmi = {
            ...
            {0}};
        HBITMAP hbm = CreateDIBSection(hdc, &bmi, DIB_RGB_COLORS, (void **)color_buffer,
                                       0, 0);
        if (hbm == 0)
            return nullptr;
