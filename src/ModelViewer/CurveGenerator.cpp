...
{
    static osg::Geometry *createCurve(Geom_BSplineCurve *curve, const osg::Vec4 &color)
    {
    ...
    static osg::Geometry *createPoints(const std::vector<osg::Vec3d> &points)
    {
        auto vertices = new osg::Vec3Array();
        for (auto &pt : points)
        {
    ...
    osg::MatrixTransform *createBSpline(const std::vector<osg::Vec3d> &ctrl_points, const std::vector<double> &params, const std::vector<osg::Vec3d> &tangents, const osg::Vec4 &color)
    {
        ...
        if (interpolate->IsDone())
        {
            ...
            geod->addDrawable(createCurve(curve.get(), color));
        }

        geod->addDrawable(createPoints(ctrl_points));
